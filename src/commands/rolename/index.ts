import { ChatInputCommandInteraction, MessageFlags, GuildMemberRoleManager, Interaction, InteractionCallback } from "discord.js"
...
export default (async ({ bot, commandDir }: { bot: Bot, commandDir: string }) => {
    return ({
        ...
        exec: async function(this: Command, interaction: ChatInputCommandInteraction) {
            ...
            if (!newRoleName) return await interaction.reply({ content: "❌ **Unable to read the new name for your role.**", flags: MessageFlags.Ephemeral })
